...
<!--        图片素材-->
        <el-upload :action="fileUrlUpload"
                   :data = upData
...
<script>
import {fileUrl,fileUrlUpload} from "/env";
...
export default {
  ...
  data(){
    return {
      ...
      uploadHeaders:{ Authorization: "Bearer " + getToken() }, // 设置上传的请求头部
      fileList:[],
      upData: {} // 上传的额外数据，用于文件名
    }
  },
  methods: {
    ...
    handleAvatarSuccess(res) {
      ...
      if (res.code !== 0){
        ...
        this.attribute.imgUrls.push(res.data);
      }
      this.attribute.imgUrls.push(fileUrlUpload);
    },
    handleError(err, file){
      this.$modal.msgError("图片上传失败，使用本地路径")
