    ...
    <el-card class="box-card" v-loading="processInstanceLoading" v-for="(item, index) in runningTasks" :key="index">
      ...
      <el-col :span="16" :offset="6" >
        <el-form :ref="'form' + index" :model="auditForms[index]" :rules="auditRule" label-width="100px">
          ...
          <el-form-item label="动态审批" prop="addAssignees">
            <el-input type="textarea" v-model="auditForms[index].addAssignees" placeholder="动态审批人" />
          </el-form-item>
        </el-form>
  ...
  methods: {
    ...
    handleAudit(task, pass) {
      ...
      this.$refs['form' + index][0].validate(valid => {
        ...
        let assignArray =  [];
        let inputAddAssignees = this.auditForms[index].addAssignees;
        if(!!inputAddAssignees) {
          assignArray = inputAddAssignees.split(",");
