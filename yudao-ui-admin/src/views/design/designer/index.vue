...
<script>
import ComponentBar from "@/views/design/designer/componentBar";
import ConfigBar from "@/views/design/designer/configBar";
import cptOptions from "@/components/DataV/options"
import SittingForm from "@/views/design/designer/sittingForm";
import html2canvas from 'html2canvas';
import env from "/env";
import {fileDownload, getByIdApi,base64toFile} from "@/api/design/DesignerApi";
import {updateScreen} from "@/api/design/screen";
import {clearCptInterval} from "@/utils/refreshCptData";
import ScaleMarkX from "@/views/design/designer/scaleMark/ScaleMarkX";
import ScaleMarkY from "@/views/design/designer/scaleMark/ScaleMarkY";
...
export default {
  ...
  data() {
    return {
      title_label : process.env.VUE_APP_TITLE + "大屏设计器",
      windowWidth:0,
      windowHeight:0,
      fileUrl:env.fileUrl,
      cptBarWidth:200,
  ...
  methods: {
      ...
      if ('preview' === env.active){
        ...
        if (cacheStr){
          ...
          if (!this.designData.version || this.designData.version !== env.version){
            localStorage.removeItem('designCache');
            this.$modal.msgSuccess("发现旧版缓存，已清除");
          }else{
        ...
        if (id){
          ...
          this.$modal.msgError('当前为DEV模式，请从管理端进入')
        }
      }
    },
    copyCpt(item){
    ...
    submitDesign() {//保存
      if ('preview'===env.active){
        ...
        html2canvas(that.$refs.webContainer, {backgroundColor: '#49586e'}).then(canvas => {
          ...
          updateScreen(this.designData).then(res2 => {
              loading.close();
    ...
    drop(e) {//从组件栏丢下组件
      ...
      if (group && group.options[config.name + '-option']) {
        ...
        this.$modal.msgError("未再options.js中查找到"+config.group+"."+config.name+"-option的自定义属性")
        return;
      }
